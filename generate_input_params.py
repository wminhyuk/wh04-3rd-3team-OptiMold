...
for run_id, files in run_id_map.items():
    ...
    try:
        ...
        if backpr_file:
            ...
            entry["Backpr_amp"] = min(amp, 2.0)

        # Retract_delay (offset from 2.06s)
        ...
        # Nozzle_delay (offset from 2.0s, clip to 1.5s)
...
df = df.sort_values(by="run_id").reset_index(drop=True)  # ✅ run_id 기준 정렬
